...
## Recent Changes
- 2026-02-26: Fixed mobile container positioning architecture. All `fixed` CSS positioning converted to `sticky` (headers) or `absolute` (modals/overlays) to keep elements within the max-w-md mobile frame. App.tsx restructured to flex column layout: scrollable content area + BottomNav as regular flex child. No `fixed` positioning anywhere in the codebase.
