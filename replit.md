...
## Recent Changes
- 2026-02-25: Full responsive design across all pages. App wrapper expanded from max-w-md to md:max-w-3xl lg:max-w-5xl xl:max-w-7xl. All full-screen pages (InvestmentsHub, LoansHub, CardManagement, PaymentFlow, SupportService, NicheLendingHub, LegacyServices, CardApplicationFlow, ProfileSection, UPIService, OnboardingFlow, AutomationHub, TransactionsPage) use max-w-4xl mx-auto centering for nav and content. PortfolioPage uses max-w-6xl. PersonaSelector uses md:grid-cols-2. Dashboard uses md:grid-cols-2/3/4 for sections. BottomNav widens with max-w-sm md:max-w-lg lg:max-w-xl. Oracle chat messages/input centered with max-w-4xl. Fixed JSX syntax issues in OracleBriefHub (spaces in tag names).
