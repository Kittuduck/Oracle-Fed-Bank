...
const UPIService: React.FC<UPIServiceProps> = ({ onBack, currentBalance, isDarkMode, festival }) => {
    ...
    return (
        <div className={`min-h-screen transition-all duration-700 font-sans flex flex-col overflow-hidden ${isDarkMode ? 'dark bg-zinc-950 text-zinc-100' : 'bg-white text-[#333333]'} ${festival !== 'DEFAULT' ? `theme-festive-${festival.toLowerCase()}` : ''}`}>
            {/* Header */}
            <div className="px-6 py-4 flex items-center justify-between border-b border-slate-100 dark:border-zinc-800 bg-white/95 dark:bg-zinc-950/90 backdrop-blur-xl sticky top-0 z-50">
                <button
